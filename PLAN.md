...
**Lipo privileged path via SweepHelper** (queued 2026-09-02, user asked for Touch ID on the thin password prompt): the shipped path escalates through `osascript … with administrator privileges`, whose system dialog is password-only — macOS offers no Touch ID hook there. Routing the root-owned-bundle thin through the existing `SweepHelper` daemon removes the per-operation prompt entirely (one-time Settings approval, Touch ID-class UX). Blocked on its own adversarial review: the helper's `MaintenanceOperation` set is a closed, Codex-reviewed contract, and adding a root app-bundle mutation (path validation, symlink escape, protocol v2 bump) must go through the same fix-verify-adjudicate loop the other gates did before the case is added.
