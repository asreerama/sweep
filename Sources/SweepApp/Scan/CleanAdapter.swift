...
struct CleanAdapter: CleanBackend {
    ...
    /// matches, runs a `CleanRequest` carrying only its receipt through `CleanService`. Adds this
    /// node's freed bytes to `freedBytes` on success; never throws — every failure mode (no rule
    /// on record, the node no longer existing or no longer matching, `CleanService` reporting a
    /// per-item failure) becomes a `.failed` outcome for this one row instead of aborting the
    /// whole request.
    ///
    ...
    private func cleanOne(
        ...
        do {
            ...
            guard
                let matchedCandidate = verificationScan.candidates.first(where: {
                    $0.url.standardizedFileURL.path == standardizedNodePath
                }),
                let receipt = verificationScan.receipt(forCandidateID: matchedCandidate.id)
            else {
                return SweepUI.CleanItemOutcome(
                    id: id, title: item.title, byteCount: item.byteCount,
            ...
            let request = SweepCore.CleanRequest(receipts: [receipt], selectedCandidateIDs: [receipt.id])
