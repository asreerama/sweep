...
/// `perform`'s `requestData` must decode strictly as ``MaintenanceOperation`` (PLAN §2: "closed
/// enum ops ONLY... NO generic command/path parameter anywhere in the protocol") and anything that
/// fails to decode as one of its three defined cases is refused, closed by construction rather
/// than by convention. A `Codable` JSON envelope over `Data` is used instead of `NSSecureCoding`
/// on the enum itself so the closed set lives in one auditable Swift type
/// (`MaintenanceOperation`), not in a hand-rolled `NSSecureCoding` implementation that could
...
@objc public protocol SweepHelperXPCProtocol {
    ...
    /// `requestData` must decode as ``MaintenanceOperation``; `reply` carries a JSON-encoded
    /// ``MaintenanceOutcome``. Never accepts or returns a path/command string of its own — see the
